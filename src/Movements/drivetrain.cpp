...
std::shared_ptr<ChassisController> Drivetrain::mdt =
    ChassisControllerBuilder()
        .withMotors({Drivetrain::mmtrLeftFront, Drivetrain::mmtrLeftMid, Drivetrain::mmtrLeftBack},
                    {Drivetrain::mmtrRightFront, Drivetrain::mmtrRightMid,Drivetrain::mmtrRightBack})
...
void Drivetrain::fDist(int dist) // Moves thr robot forward until the distance sensors reach the target
{
    ...
    def::controller.rumble("-"); // Rumbles the controller to indicate that we reached the target
    Drivetrain::moveArcade(0,0, false); 
}
...
void Drivetrain::bDist(int dist) // Moves the robot backward until the distance sensors reach the target
{
    ...
    def::controller.rumble("-"); // Rumbles the controller to indicate that the loop finished
    Drivetrain::moveArcade(0,0, false);
}
...
void Drivetrain::straightForDistance(QLength idistance, PID imagnitudePID, Slew imagnitudeSlew, PID iturnPID)
{
    ...
    while (menabled && !imagnitudePID.isSettled())
    {
        ...
        // /-iturnPID.iterate(angleError)
        Drivetrain::moveArcade(imagnitudeSlew.iterate(imagnitudePID.iterate(inError)), 0, false);
...
void Drivetrain::turnToAngle(QAngle iangle, PID ipid)
{
  ...
  // Enable the drivetrain and enter a loop that runs until the PID object is settled or the drivetrain is disabled
    ...
    while (menabled && !ipid.isSettled())
    {
        ...
        - ((def::imu_side.get_rotation()+def::imu_top.get_rotation()+def::imu_back.get_rotation()+ def::imu_fl.get_rotation() + def::imu_fr.get_rotation())/5)); // Gets the values directly in the loop, preventing infinite loops

        printf("D-Angle Error: %d", angleError);
        // Prints out the turn PID error to the terminal
        // which will help with debugging the PID as well
        // as to help tuning. 

        /* ? REQUIRES A Tethered Brain Connection ? */ 
        ...
        pros::delay(20);
        // Delays for 20 milliseconds to prevent the loop from  getting stuck
    }
